...
test('mapDst() should process the template in "dst"', t => {
    t.plan(1);
    t.equal(mapDst(null, null, null, {attrs: {dst: 'c:\\<%= test %>\\cica.txt'}}), 'c:\\test\\cica.txt');
});

test('mapDst() should add a file name if "dst" is a folder', t => {
    t.plan(2);
    t.equal(mapDst('cica.component', null, {text: '.txt'}, {name: 'text', attrs: {dst: 'c:\\test'}}), 'c:\\test\\cica.txt');
    t.equal(mapDst('cica.component', null, null, {processor: {ext: '.txt'}, attrs: {dst: 'c:\\test'}}), 'c:\\test\\cica.txt');
});

test('mapDst() should use "dstBase"', t => {
    t.plan(1);
    t.equal(mapDst('cica.component', 'c:\\', null, {attrs: {dst: 'test\\cica.txt'}}), 'c:\\test\\cica.txt');
});

test('mapDst() should not use "dstBase" if "dst" is absolute', t => {
    t.plan(1);
    t.equal(mapDst('cica.component', 'd:\\', null, {attrs: {dst: 'c:\\test\\cica.txt'}}), 'c:\\test\\cica.txt');
});
