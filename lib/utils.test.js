...
const
    {forEachAsync, filterAsync, isFile, fileNameWithoutExtension, isDefined, strip, countLinesTo, merge} = require('./utils'),
...
test('merge() test', t => {
    ...
    var dst = {foo: 'bar'};
    t.deepEqual(merge({}, dst), {foo: 'bar'});
    t.equal(merge({}, dst), dst);

    dst = {};
    t.deepEqual(merge({foo: 'bar'}, dst), {foo: 'bar'});
    t.equal(merge({foo: 'bar'}, dst), dst);

    dst = {foo: 'bar', cica: 'kutya'};
    t.deepEqual(merge({foo: 'qux'}, dst), {foo: 'qux', cica: 'kutya'});

    dst = {foo: 'qux'};
    t.deepEqual(merge({foo: 'bar', cica: 'kutya'}, dst), {foo: 'bar', cica: 'kutya'});
});
